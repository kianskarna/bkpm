		...
		if ($count > 0 ) {
			...
			foreach($result as $row)
			{
				if($row['status'] == 'inactive')
				{
					...
					$date = date("Y-m-d H:i:s");
					$query = "
					INSERT INTO data_kegiatan_selesai (kegiatan_dimulai_id, foto_kegiatan_selesai,user_id ,waktu_kegiatan_selesai) VALUES ('".$_POST["kegiatan_dimulai_id"]."','".$image."','".$_SESSION["user_id"]."','".$date."') 
					";
					$statement = $connect->prepare($query);
					$statement->execute();
					$result = $statement->fetchAll();
